...
function AchievementCard({ achievement, onClaim }) {
  ...
  const cardClass = 'bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-slate-100 dark:border-slate-800/60 rounded-xl shadow-sm text-slate-900 dark:text-white transition-colors duration-300 p-4 flex flex-col gap-4';
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div>
              <h3 className="text-base font-black text-slate-900 dark:text-white leading-tight">{achievement.title}</h3>
    ...
    <div className="max-w-6xl mx-auto page-enter">
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Earn badges by completing quizzes, improving scores, and answering more questions.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div key={label} className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-slate-100 dark:border-slate-800/60 rounded-xl shadow-sm text-slate-900 dark:text-white transition-colors duration-300 p-4"
            style={{ border: '1.5px solid rgba(102,126,234,0.12)', boxShadow: '0 1px 5px rgba(99,102,241,0.08)' }}>
            <div className="text-2xl font-black text-slate-900 dark:text-white">{value}</div>
            <div className="text-xs text-slate-500 dark:text-slate-400 font-bold mt-1">{label}</div>
          </div>
        ))}
      </div>

      {claimed.length > 0 && (
        <div className="bg-white dark:bg-slate-900/50 backdrop-blur-sm border border-slate-100 dark:border-slate-800/60 rounded-xl shadow-sm text-slate-900 dark:text-white transition-colors duration-300 p-5 mb-5"
          style={{ border: '1.5px solid rgba(102,126,234,0.12)', boxShadow: '0 1px 5px rgba(99,102,241,0.08)' }}>
          <h3 className="text-lg font-black text-slate-900 dark:text-white mb-3">Badge Collection</h3>
